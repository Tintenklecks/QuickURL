...
class URLManagerViewModel: ObservableObject {
    ...
    /// Stores security-scoped bookmark data for a URL item
    func updateBookmark(for itemId: UUID, bookmarkData: Data?) {
        guard let index = urlItems.firstIndex(where: { $0.id == itemId }) else { return }
        urlItems[index].bookmarkData = bookmarkData
        saveURLs()
    }
